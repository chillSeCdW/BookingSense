...
struct OpenTimelineEntryEntity: AppEntity {
  static var typeDisplayRepresentation: TypeDisplayRepresentation = "Timeline Entry"
  static var defaultQuery = OpenTimelineEntryQuery()
...
struct OpenTimelineEntryQuery: EntityQuery {
  func entities(for identifiers: [String]) async throws -> [OpenTimelineEntryEntity] {
    ...
    return entries.map { OpenTimelineEntryEntity(from: $0) }
  }

  func suggestedEntities() async throws -> [OpenTimelineEntryEntity] {
    let context = ModelContext(DataModel.shared.modelContainer)
    let timelineEntryStateOpen = TimelineEntryState.open.rawValue

    var descriptor = FetchDescriptor<BookingSchemaV5.TimelineEntry>(
      predicate: #Predicate {
        $0.completedAt == nil && $0.state == timelineEntryStateOpen
      },
      sortBy: [.init(\.isDue)]
    )
